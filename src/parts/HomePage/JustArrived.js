...
export default function JustArrived() {
  ...
  useEffect(() => {
    run(
      fecth({url:"/api/products/?page=1&limit=10"})
    )
  }, [run])
  ...
  return (
    ...
    <div className="overflow-x-hidden px-4" id="carousel">
      ...
      <div className="overflow-hidden z-10">
        ...
        {
          isLoading ? <div className="flex -mx-4 flex-row relative" style={{ paddingLeft: refContainer.current?.getBoundingClientRect?.()?.left - 16 || 0 }}><Loading/> </div>
          : error ? (JSON.stringify(error)) :
           data.data.length === 0? ("no product found") :
           ( 
          <Carousel redContainer ={refContainer}>
            {
               data.data.map((item) =>{
                return(
