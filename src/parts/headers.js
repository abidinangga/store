...
export default function headers({theme,position}) {
  useState [ToggleMainMenu , setToggleMainMenu] = useState(false)
  useState [isCartChanged , setCartChanged] = useState(false)
        ...
        <div className="flex flex-stretch items-center">
          ...
          <div className="w-auto">
            <ul className="items-center flex">
              ...
              <li className="ml-6">
                <Link
                  className={["cart flex items-center justify-center w-8 h-8", theme === "white" ? " text-black md:text-white" : " text-white md:text-black",state.cart && object.keys(state.cart).length > 0 ? "cart-filled"
                : "", isCartChanged ? "animate-bounce" : "", ].join(" ")}
                  to="/cart"
                >
                  <iconCart></iconCart>
                </Link>
